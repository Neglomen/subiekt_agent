...
**Wersja aplikacji:** 0.11.0 — jedyne źródło prawdy to `app/services/updater.py::APP_VERSION` (importują je `app/main.py` i `app/gui_api.py`). Drugie, niezależne miejsce to `AppVersion` w `installer/setup.iss` — Inno Setup nie zaimportuje stałej z Pythona, więc przy wydaniu trzeba podbić oba.
